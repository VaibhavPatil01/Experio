...
const AIResumeAnalyser = () => {
  ...
  const handleReanalyze = async (historyItem) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_API_URL || 'http://localhost:3000'}/api/resume-analyzer/${historyItem._id}/reanalyze`,
        {
  ...
  const handleDelete = async (historyItem, e) => {
    ...
    try {
      await axios.delete(
        `${import.meta.env.VITE_BASE_API_URL || 'http://localhost:3000'}/api/resume-analyzer/${historyItem._id}`,
        {
          headers: { token: getAuthToken() },
          withCredentials: true
        }
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_API_URL || 'http://localhost:3000'}/api/resume-analyzer/analyze`,
        formData,
        {
          headers: {
            ...
            >
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <div
                    className={`relative border-2 border-dashed rounded-xl p-8 text-center transition-all duration-200 ${isDragActive
                      ...
                      {result.result.priorityRecommendations.map((rec, index) => (
                        <div key={index} className="bg-gray-50 dark:bg-[#252525] rounded-lg p-5 border border-gray-100 dark:border-gray-800">
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <span className={`text-xs font-bold uppercase tracking-wider px-2 py-1 rounded-md ${rec.priority?.toLowerCase() === 'high' ? 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400' :
